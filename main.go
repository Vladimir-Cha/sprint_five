...
func (t Training) TrainingInfo() InfoMessage {
	...
	return InfoMessage{
		...
		Distance:     t.distance(),   //расстояние
		Speed:        t.meanSpeed(),  //скорость
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	runCalories := ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours)
	return runCalories
}
...
func (w Walking) Calories() float64 {
	...
	walkCalories := ((CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed(), 2)/(w.Height/CmInM))*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
	return walkCalories
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	swimCalories := (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
	return swimCalories
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	...
	return InfoMessage{
		...
		Duration:     s.Duration,     //длительность
		Distance:     s.distance(),   //расстояние
		Speed:        s.meanSpeed(),  //скорость
		Calories:     s.Calories(),   //калории
	}
}
